...
st.set_page_config(
    page_title="QLAB Lifetime tools",
    page_icon="👋",
)

st.title("TCSPC fitting")
st.markdown('TThis research has received funding support from the NSRF via the Program Management Unit for Human Resources & Institutional Development, Research and Innovation [grant number B13F670064]')
st.markdown('Example data can be found here: https://github.com/yok3leg/TCSPC_fitting/tree/PMUB/Example%20Data')
st.sidebar.image("https://pmu-hr.or.th/wp-content/uploads/2024/01/02_PMUB-PNG.png")

#### Upload area####
uploaded_files = st.file_uploader("Choose TCSPC data (.txt)", accept_multiple_files=True) # create upload box
if uploaded_files: # if there is/are file(s) uploaded
    ...
    #### Time alignment ####
    en_shift = st.sidebar.checkbox('Peak Alignment')
    peak_temp = 0
